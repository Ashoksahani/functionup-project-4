...
const redisClient = redis.createClient(
  17866,
  "redis-17866.c212.ap-south-1-1.ec2.cloud.redislabs.com",
  
  { no_ready_check: true }
);
redisClient.auth("05WmsULsVKD457nRc5al18MymmmmF9Lu", function (err) {
  if (err) throw err;
});
  ...
  try{
    ...
    //try {
      ...
      if (urlDataFromCatch) {
        ...
        res.status(200).send({status: true,message: "Url shorten successfully",data: data,});
      } else {
    ...
    //try {
      ...
      if (urlDataFromCatch) {
        ...
        if (!urlDataByUrlCode) {
          return res
            .status(404)
            .send({ status: false, message: "No such url exist please enter valid urlcode" });
        }
